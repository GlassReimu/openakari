...
akari sessions are human-initiated: a person runs Claude Code, picks a task, and works. The repo has strong conventions for within-session discipline (inline logging, provenance, decision records) but no mechanism for sessions to happen without a human trigger.
...
- Session metrics accumulate in log entries, feeding the akari meta-project's research on session efficiency.
