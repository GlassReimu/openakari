...
The akari repo has approval gates for resource decisions but no project-level budget tracking. Agents can freely consume resources across sessions without knowing the cumulative total or remaining allocation. As projects scale (e.g., a sample project planning a 4,320-call full evaluation), there's no mechanism to say "this project has X calls and must finish by date Y" and have that enforced.
