...
3. Experiments were split across two different Cloudflare AI Gateway accounts (`research` and `akari`), and only one was API-queryable.
...
1. **Single gateway**: All API calls (LLM and 3D generation) route through the `research` CF gateway (account `<your-cloudflare-account-id>`). The `akari` gateway is retired.
...
- The `akari` gateway's historical logs are inaccessible — those calls are unverifiable.
