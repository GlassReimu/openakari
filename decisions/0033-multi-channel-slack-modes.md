...
Akari's Slack bot operated in a single-user DM model: one designated user (`SLACK_USER_ID`) with full access to all capabilities. As Akari matures and begins collaborative work with human teams, it needs to be present in Slack channels where multiple users interact with it in different contexts.
...
- Akari can read files to answer accurately but cannot use Edit, Write, Bash, or other mutating tools
...
1. **Slash command**: `/akari mode <dev|chat>` — run in any channel where the bot is present to set that channel's mode. Only the designated user (`SLACK_USER_ID`) or Slack workspace admins can change modes.
2. **App Home**: The Akari Slack app's Home tab displays all configured channels with their current modes. The designated user can add, remove, or change channel modes from this view.
...
4. **Fallback**: If a channel is not in the config file and the bot is invited to it, the bot ignores messages until a mode is set via `/akari mode`.
...
- Slash command handler (`/akari mode`) — validates permissions, updates registry, persists to file, responds with confirmation.
- App Home tab — renders current channel mode configuration using Block Kit. Includes buttons to change or remove modes.

Modified modules:
- `slack.ts` — message handler checks channel context (DM vs channel, registered vs unregistered) and routes accordingly. Added `app_mention` handler for channel @mentions. Unregistered channels are silently ignored. Registers the `/akari` slash command and App Home event handler.
...
- Akari can now be added to team Slack channels with appropriate access levels.
...
- The Slack app requires additional scopes (`commands` for slash commands, `app_mentions:read`) and a registered slash command (`/akari`).
...
1. Register the `/akari` slash command in the Slack app configuration (api.slack.com) pointing to the bot's request URL
...
5. Create an empty `infra/slack-bot/channel-modes.json` (`{"channels": {}}`) — or let the bot create it on first `/akari mode` invocation
6. Invite the bot to desired channels, then run `/akari mode dev` or `/akari mode chat` in each channel to configure its mode
