...
**Throughput vs. Coordination** — akari's current serialized architecture (ADR 0005) maximizes coordination at the cost of throughput. A single agent produces one stream of findings per hour. GLM-5 FP8 offers massive parallel capacity at zero marginal API cost, but 32 concurrent agents risk git conflicts, duplicated work, and incoherent knowledge output.

This proposal moves akari toward the **throughput** side of the tension while introducing coordination mechanisms that preserve knowledge coherence. The tradeoff: more complexity in orchestration, but 32x capacity with no additional API spend.
...
| `event-agents.ts:343` | Slack `/akari work` | deepWork | opus | 256 | 60 min | Task-specific deep work |
| `event-agents.ts:610` | Slack `/akari team` | teamWorkSession | opus | 256 | 120 min | Team-based deep work |
...
**Implement a Persistent Subagent Fleet**: 32 opencode/GLM-5 agents running continuously (24/7), each assigned to a dedicated "workstream" — a partitioned slice of akari's mission.
...
workstreams:
  ...
  - id: literature-huggingface
    type: guaranteed-infinite
    focus: "Monitor huggingface models/datasets. Evaluate relevance to akari projects."
    projects: [all]
    agents: 2
  ...
  - id: other-projects
    ...
    projects: [akari-overview, other-projects...]
    agents: 4
...
**Positive:**
- 32x increase in akari's throughput at zero marginal API cost
...
- Fleet-level metrics provide new visibility into akari's operation
