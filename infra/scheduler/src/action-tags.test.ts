...
describe("findActionTag — run_burst", () => {
  it("parses run_burst with all parameters", () => {
    const text = `Starting burst! [ACTION:run_burst job="akari-work-cycle" max_sessions=5 max_cost=15 autofix=true]`;
    const result = findActionTag(text);
    expect(result).not.toBeNull();
    expect(result!.kind).toBe("run_burst");
    expect(result!.params.job).toBe("akari-work-cycle");
...
describe("eagerlySetPendingAction — run_burst", () => {
  it("sets pendingAction for run_burst", () => {
    const parsed = findActionTag(`[ACTION:run_burst job="akari-work-cycle" max_sessions=5 max_cost=15 autofix=true]`);
    const action = eagerlySetPendingAction(parsed!);
    expect(action).not.toBeNull();
    expect(action!.kind).toBe("run_burst");
    expect(action!.jobId).toBe("akari-work-cycle");
...
describe("buildConfirmPrompt — run_burst", () => {
  it("generates confirmation prompt for burst", () => {
    const prompt = buildConfirmPrompt({
      kind: "run_burst",
      jobId: "akari-work-cycle",
      maxSessions: 10,
      maxCost: 20,
      autofix: true,
    });
    expect(prompt).toContain("akari-work-cycle");
...
describe("stripActionTags — run_burst", () => {
  it("strips run_burst tags from text", () => {
    const text = `Burst starting! [ACTION:run_burst job="akari-work-cycle" max_sessions=5 max_cost=15 autofix=true]`;
...
describe("findActionTag — create_task", () => {
  it("parses create_task with all fields", () => {
    const text = `Creating task! [ACTION:create_task project="akari" task="Write log entry" done_when="Log entry exists in README"]`;
    const result = findActionTag(text);
    expect(result).not.toBeNull();
    expect(result!.kind).toBe("create_task");
    expect(result!.params.project).toBe("akari");
  ...
  it("returns null for missing done_when", () => {
    const text = `[ACTION:create_task project="akari" task="Write log"]`;
    const result = findActionTag(text);
...
describe("eagerlySetPendingAction — create_task", () => {
  it("returns null for create_task (immediate, no confirmation)", () => {
    const parsed = findActionTag(`[ACTION:create_task project="akari" task="Write log" done_when="Log exists"]`);
    const action = eagerlySetPendingAction(parsed!);
    expect(action).toBeNull();
  });
});
...
describe("stripActionTags — create_task", () => {
  it("strips create_task tags from text", () => {
    const text = `Task created! [ACTION:create_task project="akari" task="Write log" done_when="Log exists"]`;
    const stripped = stripActionTags(text);
...
describe("findAllActionTags — multiple action tags", () => {
  it("returns all matches from text with 2+ tags", () => {
    const text = `First action. [ACTION:create_task project="akari" task="Task one" done_when="Done"] Second action. [ACTION:create_task project="akari" task="Task two" done_when="Done"]`;
  ...
  it("returns all matches for different action types", () => {
    const text = `[ACTION:create_task project="akari" task="New task" done_when="Done"] [ACTION:restart] [ACTION:fleet_control op="enable" size=2]`;
    const results = findAllActionTags(text);
...
describe("findAllActionTags — single action tag", () => {
  it("returns single-element array for text with 1 tag", () => {
    const text = `Starting task! [ACTION:create_task project="akari" task="Single task" done_when="Complete"]`;
    const results = findAllActionTags(text);
...
describe("findAllActionTags — multiple suggest_task/note_question tags", () => {
  it("returns all suggest_task tags from multi-action response", () => {
    const text = `Great ideas! [ACTION:suggest_task project="akari" task="Add batch processing"] Also: [ACTION:suggest_task project="akari" task="Improve error handling"]`;
    const results = findAllActionTags(text);
    expect(results).toHaveLength(2);
  ...
  it("returns mixed suggest_task and note_question tags", () => {
    const text = `[ACTION:suggest_task project="akari" task="New feature"] Also a question: [ACTION:note_question project="akari" question="Why does X fail?"]`;
    const results = findAllActionTags(text);
    expect(results).toHaveLength(2);
    expect(results[0].kind).toBe("suggest_task");
  ...
  it("can filter chat-mode actions from mixed response", () => {
    const text = `[ACTION:suggest_task project="akari" task="Feature A"] [ACTION:create_task project="akari" task="Task B" done_when="Done"] [ACTION:note_question project="akari" question="Q1"]`;
