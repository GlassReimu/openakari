...
describe("buildAutoCommitArgs", () => {
  ...
  it("returns file list for orphaned work artifacts", () => {
    const result = buildAutoCommitArgs(
      [" M projects/akari/README.md", "?? projects/sample-project/experiments/done-exp/results/out.csv"],
      [],
    );
    expect(result).not.toBeNull();
    expect(result!.files).toContain("projects/akari/README.md");
  ...
  it("handles mixed orphaned and expected files", () => {
    const result = buildAutoCommitArgs(
      [
        " M projects/akari/README.md",        // orphaned (modified tracked file)
  ...
  it("excludes modified tracked files inside active experiment dirs", () => {
    ...
    expect(result!.files).toEqual(["projects/akari/README.md"]);
