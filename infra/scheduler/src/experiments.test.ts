...
describe("normalizeShellCommand", () => {
  ...
  it("wraps commands with && operator in bash -c", () => {
    ...
    const cmd = ["cd", "/home/user/akari/projects/foo", "&&", "export", "SEDASHIM_ANYCALL_ENDPOINT=staging", "&&", "python3", "run.py"];
    const result = normalizeShellCommand(cmd);
    expect(result).toEqual(["bash", "-c", "cd /home/user/akari/projects/foo && export SEDASHIM_ANYCALL_ENDPOINT=staging && python3 run.py"]);
  });
