...
function makeStateFile(content: object): string {
  const path = join(tmpdir(), `akari-health-test-${Date.now()}-${Math.random().toString(36).slice(2)}.json`);
...
describe("runHealthCheck", () => {
  ...
  beforeEach(() => {
    stateFile = join(tmpdir(), `akari-health-test-${Date.now()}-${Math.random().toString(36).slice(2)}.json`);
  });
